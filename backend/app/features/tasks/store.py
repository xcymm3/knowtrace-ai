...
class SupabaseTaskStore:
    ...
    def get_task(self, task_id: UUID) -> dict[str, Any]:
        response = self._client.table("research_tasks").select("*").eq("id", str(task_id)).execute()
    ...
    def update_task(self, task_id: UUID, data: dict[str, Any]) -> None:
        self._client.table("research_tasks").update(data).eq("id", str(task_id)).execute()

    def update_document(self, document_id: UUID, data: dict[str, Any]) -> None:
        self._client.table("source_documents").update(data).eq("id", str(document_id)).execute()
